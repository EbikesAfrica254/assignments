package com.ebikes.assignments.database.specifications;
...
import com.ebikes.assignments.support.fixtures.NotificationRequestFixtures;
import com.ebikes.assignments.support.infrastructure.AbstractRepositoryTest;
...
class OutboxSpecificationsTest extends AbstractRepositoryTest {
  ...
  private static final String EVENT_OFFER_CREATED = "OFFER_CREATED";
  private static final String EVENT_OFFER_UPDATED = "OFFER_UPDATED";
  ...
  private Outbox failed() {
    Outbox outbox = pending(OutboxSpecificationsTest.EVENT_OFFER_CREATED);
    outbox.markFailed();
    return outbox;
  }

  private Outbox sent() {
    Outbox outbox = pending(OutboxSpecificationsTest.EVENT_OFFER_UPDATED);
    outbox.markSent();
    return outbox;
  }
  ...
  void setUp() {
    repository.saveAll(
        List.of(
            pending(EVENT_OFFER_CREATED),
            pending(EVENT_OFFER_UPDATED),
            failed(),
            sent(),
            failedWithRetries()));
  }
  ...
  class FilterByEventType {
    ...
    void shouldReturnOnlyMatchingEventType() {
      OutboxFilter filter = new OutboxFilter();
      filter.setEventType(EVENT_OFFER_CREATED);

      List<Outbox> results = repository.findAll(OutboxSpecifications.buildSpecification(filter));

      assertThat(results).hasSize(3).allMatch(o -> o.getEventType().equals(EVENT_OFFER_CREATED));
    }
  ...
  class CombinedFilters {
    ...
    void statusAndEventTypeCombinedReturnsCorrectSubset() {
      ...
      assertThat(results)
          ...
          .satisfies(
              o -> {
                assertThat(o.getEventType()).isEqualTo(EVENT_OFFER_CREATED);
